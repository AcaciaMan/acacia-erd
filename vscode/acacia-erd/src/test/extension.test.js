...
function createVscodeStub() {
    ...
    const mock = {
        commands: {
            registerCommand: sinon.stub().callsFake((id, cb) => {
        ...
        window: {
            ...
            registerWebviewViewProvider: sinon.stub().callsFake((viewId, provider) => {
        ...
        workspace: {
            getConfiguration: sinon.stub().returns({
                get: sinon.stub().callsFake((key, defaultValue) => {
                    if (key === 'entitiesJsonPath') {
                        return 'resources/entities.json';
                    }
                    if (key === 'sourceFolders') {
...
suite('Extension', () => {
    ...
    setup(() => {
        ...
        mockELM = {
            ...
            resolveAbsolutePath: sinon.stub().callsFake((list) => list.jsonPath),
            onDidChange: sinon.stub().returns({ dispose: sinon.stub() }),
            dispose: sinon.stub(),
        };
        mockEntityManager = {
        ...
        const mod = proxyquire('../extension', {
            ...
            './manage_erd/ERDViewProvider': {
                ERDViewProvider: class MockERDViewProvider {
                    constructor(_ctx, _sfm, _dbm, _elm) { }
                },
                '@noCallThru': true,
            },
    ...
    suite('activate()', () => {
        ...
        test('exportInteractiveHtml with no open panel shows warning message', async () => {
            ...
            assert.ok(vscodeKit.mock.window.showWarningMessage.firstCall.args[0]
        ...
        test('creates tree view for acaciaAssets', () => {
            ...
            const assetsCall = calls.find((c) => c.args[0] === 'acaciaAssets');
        ...
        test('assets tree view is created with showCollapseAll: true', () => {
            ...
            const calls = vscodeKit.mock.window.createTreeView.getCalls();
            const assetsCall = calls.find((c) => c.args[0] === 'acaciaAssets');
            assert.strictEqual(assetsCall.args[1].showCollapseAll, true, 'acaciaAssets should have showCollapseAll: true');
        });
        test('tree view disposable is pushed to context.subscriptions', () => {
            ...
            for (const disposable of treeViewDisposable) {
                assert.ok(vscodeKit.subscriptions.includes(disposable), 'tree view disposable should be in context.subscriptions');
            }
        });
        test('tree view provider has TreeDataProvider interface', () => {
            ...
            assert.strictEqual(typeof provider.getTreeItem, 'function', 'assets provider should implement getTreeItem');
            assert.strictEqual(typeof provider.getChildren, 'function', 'assets provider should implement getChildren');
            assert.strictEqual(typeof provider.refresh, 'function', 'assets provider should implement refresh');
        });
