...
function createMockWebviewPanel() {
    ...
    const panel = {
        ...
        onDidChangeViewState: sinon.stub().callsFake((_cb) => {
            return { dispose: sinon.stub() };
        }),
        visible: true,
    };
...
function createFsStub(htmlContent = DUMMY_HTML) {
    return {
...
function createVscodeStub(primaryPanel, secondaryPanel) {
    let callCount = 0;
    return {
        window: {
...
function createMockEntityManager() {
    ...
    const mgr = {
        ...
        getEntityByName: sinon.stub().callsFake((name) => {
            if (name === 'User') {
...
function createMockDiagramManager(diagrams = []) {
    return {
        getDiagram: sinon.stub().callsFake((id) => diagrams.find((d) => d.id === id)),
        getDiagrams: sinon.stub().returns(diagrams),
        addDiagram: sinon.stub().callsFake((name, entityIds, positions) => {
...
function createMockEntitiesListManager(lists = []) {
    return {
        getLists: sinon.stub().returns(lists),
        resolveAbsolutePath: sinon.stub().callsFake((l) => l.jsonPath.startsWith('/') ? l.jsonPath : `/mock/workspace/${l.jsonPath}`),
...
suite('InteractiveERDPanel', () => {
    ...
    let panelKit;
    let editPanelKit;
    let describeCreateOrShow;
    let erdGenCreateOrShow;
    let htmlExporterMock;
    setup(() => {
    ...
    suite('Message handling', () => {
        test('entityClicked → calls showInformationMessage with entity name', async () => {
            ...
            assert.ok(vsMock.window.showInformationMessage.firstCall.args[0].includes('User'));
        });
        test('saveEntity → calls EntityManager.updateEntity() and posts updateEntity message', async () => {
            ...
            const updateMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'updateEntity');
        ...
        test('loadSVG → calls showOpenDialog, reads SVG file, posts loadSVGContent', async () => {
            ...
            fsStub.readFileSync.withArgs('/tmp/loaded.svg', 'utf8').returns('<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve"><rect/></svg>');
            panelKit.webview.postMessage.resetHistory();
            await panelKit.fireMessage({ command: 'loadSVG' });
            assert.ok(vsMock.window.showOpenDialog.calledOnce);
            const loadMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'loadSVGContent');
        ...
        test('loadSVG → user cancels dialog → no file read', async () => {
            ...
            const svgReads = fsStub.readFileSync.args.filter((a) => typeof a[0] === 'string' && a[0].endsWith('.svg'));
    ...
    suite('deleteEntity() (public method)', () => {
        test('posts deleteEntity message with entityName to webview', () => {
            ...
            const msg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'deleteEntity');
    ...
    suite('Path sync', () => {
        test('subscribes to EntityManager.onDidChangeEntitiesPath', () => {
            InteractiveERDPanel.createOrShow('/ext');
            assert.ok(emMock.mgr.onDidChangeEntitiesPath.calledOnce, 'should subscribe to onDidChangeEntitiesPath');
        });
        test('onDidChangeEntitiesPath → posts loadEntitiesList to webview', () => {
            ...
            const pathMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'loadEntitiesList');
    ...
    suite('openDiagram()', () => {
        ...
        test('creates panel if none exists and posts loadDiagram message', () => {
            ...
            assert.ok(vsMock.window.createWebviewPanel.calledOnce);
            const loadMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'loadDiagram');
        ...
        test('reuses existing panel if already open', () => {
            InteractiveERDPanel.createOrShow('/ext');
            vsMock.window.createWebviewPanel.resetHistory();
            const diagramMgr = createMockDiagramManager([sampleDiagram]);
            InteractiveERDPanel.openDiagram('/ext', sampleDiagram, 'Main Schema', diagramMgr);
        ...
        test('updates panel title to include diagram name', () => {
            const diagramMgr = createMockDiagramManager([sampleDiagram]);
            InteractiveERDPanel.openDiagram('/ext', sampleDiagram, 'Main Schema', diagramMgr);
            assert.ok(panelKit.panel.title.includes('Overview'));
        });
        test('sets internal diagram state', () => {
            const diagramMgr = createMockDiagramManager([sampleDiagram]);
            InteractiveERDPanel.openDiagram('/ext', sampleDiagram, 'Main Schema', diagramMgr);
    ...
    suite('saveDiagram message handling', () => {
        ...
        test('saveDiagram updates diagram via DiagramManager and writes SVG file', async () => {
            ...
            const svgPath = fsStub.writeFileSync.lastCall.args[0];
        ...
        test('saveDiagram shows info message on success', async () => {
            ...
            assert.ok(vsMock.window.showInformationMessage.calledOnce);
            assert.ok(vsMock.window.showInformationMessage.firstCall.args[0].includes('Overview'));
        });
    ...
    suite('saveAsDiagram message handling', () => {
        test('saveAsDiagram prompts for name and creates new diagram', async () => {
            ...
            InteractiveERDPanel.openDiagram('/ext', { id: 'existing', name: 'Existing', entityIds: [], positions: {} }, 'TestList', diagramMgr, undefined, mockListMgr);
            vsMock.window.showInputBox = sinon.stub().resolves('New Overview');
            fsStub.writeFileSync.resetHistory();
            await panelKit.fireMessage({
            ...
            assert.ok(vsMock.window.showInputBox.calledOnce);
        ...
        test('saveAsDiagram cancels when user dismisses name input', async () => {
            const diagramMgr = createMockDiagramManager();
            InteractiveERDPanel.openDiagram('/ext', { id: 'existing', name: 'Existing', entityIds: [], positions: {} }, 'TestList', diagramMgr);
            vsMock.window.showInputBox = sinon.stub().resolves(undefined);
            await panelKit.fireMessage({
                command: 'saveAsDiagram',
                entityIds: ['1'],
                positions: { '1': { x: 10, y: 20 } },
        ...
        test('saveAsDiagram transitions panel to diagram mode', async () => {
            const diagramMgr = createMockDiagramManager();
            InteractiveERDPanel.openDiagram('/ext', { id: 'existing', name: 'Existing', entityIds: [], positions: {} }, 'TestList', diagramMgr);
            vsMock.window.showInputBox = sinon.stub().resolves('My Diagram');
            await panelKit.fireMessage({
                command: 'saveAsDiagram',
                entityIds: ['1'],
                positions: { '1': { x: 10, y: 20 } },
        ...
        test('saveAsDiagram posts diagramInfo to webview after creation', async () => {
            const diagramMgr = createMockDiagramManager();
            InteractiveERDPanel.openDiagram('/ext', { id: 'existing', name: 'Existing', entityIds: [], positions: {} }, 'TestList', diagramMgr);
            vsMock.window.showInputBox = sinon.stub().resolves('NewDiag');
            panelKit.webview.postMessage.resetHistory();
            await panelKit.fireMessage({
                command: 'saveAsDiagram',
                entityIds: ['1'],
            ...
            const diagramInfoMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'diagramInfo');
    ...
    suite('getDiagramState message handling', () => {
        test('getDiagramState replies with diagramInfo when diagram is loaded', async () => {
            ...
            const diagMsg = panelKit.webview.postMessage.args.find((a) => a[0].command === 'diagramInfo');
...
//# sourceMappingURL=InteractiveERDPanel.test.js.map