...
function createFsStub(fileContents: string = JSON.stringify(sampleEntities)) {
    return {
 ...
 */
function loadEntityManager(fsStub: any, vscodeMock: any) {
...
suite('EntityManager', () => {
    ...
    suite('Constructor & Singleton', () => {
        ...
        test('constructor resolves relative path against workspace folder', () => {
            ...
            const calledPath = fsStub.readFileSync.firstCall.args[0] as string;
            const expectedPath = path.resolve(MOCK_WORKSPACE, 'resources/entities.json');
            assert.ok(
                calledPath === expectedPath,
                `Expected workspace-resolved path ${expectedPath}, got: ${calledPath}`
            );
        });
    ...
    suite('loadEntities()', () => {
        ...
        test('handles missing file — sets entities to [], shows error message', () => {
            ...
            assert.ok(
                (vscodeKit.mock.window.showErrorMessage.firstCall.args[0] as string)
                    .includes('ENOENT')
            );
        });
    ...
    suite('getEntityByName()', () => {
        ...
        test('throws Error with descriptive message when not found', () => {
            const mgr = EntityManager.getInstance();
            assert.throws(
                () => mgr.getEntityByName('NonExistent'),
                (err: Error) => {
    ...
    suite('addEntity()', () => {
        ...
        test('only stores Entity type fields', () => {
            ...
            const writtenJson = JSON.parse(fsStub.writeFileSync.firstCall.args[1]);
            const added = writtenJson.find((e: any) => e.name === 'Widget');
            assert.ok(added);
    ...
    suite('updateEntity()', () => {
        test('updates existing entity fields and saves', () => {
            ...
            assert.ok(fsStub.writeFileSync.calledOnce);
            const writtenJson = JSON.parse(fsStub.writeFileSync.firstCall.args[1]);
            const user = writtenJson.find((e: any) => e.name === 'User');
        ...
        test('handles entity rename — old name replaced with new name', () => {
            ...
            assert.ok(fsStub.writeFileSync.calledOnce);
            const writtenJson = JSON.parse(fsStub.writeFileSync.firstCall.args[1]);
            assert.ok(writtenJson.find((e: any) => e.name === 'Person'));
            assert.ok(!writtenJson.find((e: any) => e.name === 'User'));
        });
        ...
        test('if old entity not found during rename, delegates to addEntity()', () => {
            ...
            assert.ok(fsStub.writeFileSync.calledOnce);
            const writtenJson = JSON.parse(fsStub.writeFileSync.firstCall.args[1]);
            assert.ok(writtenJson.find((e: any) => e.name === 'Brand'));
        });
        ...
        test('if updated entity not found (same name), delegates to addEntity()', () => {
            ...
            assert.ok(fsStub.writeFileSync.calledOnce);
            const writtenJson = JSON.parse(fsStub.writeFileSync.firstCall.args[1]);
            assert.ok(writtenJson.find((e: any) => e.name === 'Ghost'));
        });
        ...
        test('calls notifyChange() after successful update', () => {
            ...
            mgr.updateEntity(
                { name: 'User', description: 'changed', columns: [], linkedEntities: [] },
                { name: 'User' }
            );
            assert.ok(changeSpy.called);
        });
    ...
    suite('deleteEntity()', () => {
        ...
        test('entity is gone from getEntities() after deletion', () => {
            ...
            assert.ok(!remaining.find((e: any) => e.name === 'Order'));
        });
    ...
    suite('setEntitiesJsonPath()', () => {
        test('calls vscode.workspace.getConfiguration().update() with new path', () => {
            ...
            const configObj = vscodeKit.mock.workspace.getConfiguration.returnValues[
                vscodeKit.mock.workspace.getConfiguration.callCount - 1
            ];
        ...
        test('stores workspace-relative path when file is inside workspace', async () => {
            ...
            const storedPath = configObj.update.firstCall.args[1] as string;
        ...
        test('re-creates file watcher after path change', async () => {
            ...
            assert.ok(
                vscodeKit.mock.workspace.createFileSystemWatcher.callCount > initialCallCount,
                'Expected file watcher to be re-created'
            );
        });
    });
