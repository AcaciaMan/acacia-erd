...
function createFsStub(fileContent) {
...
function loadDimensionManager(fsStub, vscodeMock) {
...
suite('DimensionManager', () => {
    ...
    suite('getDimensions()', () => {
        ...
        test('returns 3 built-in dimensions in seed data (level, environment, schema)', () => {
            ...
            assert.ok(dims.every((d) => d.builtIn === true));
            manager.dispose();
        });
    });
    // ── getDimension() ──────────────────────────────────────────────
    ...
    suite('renameValue()', () => {
        test('renames a value within a dimension', () => {
            ...
            const dim = manager.getDimension('custom-dim');
            const val = dim.values.find((v) => v.id === 'val-a');
...
//# sourceMappingURL=DimensionManager.test.js.map