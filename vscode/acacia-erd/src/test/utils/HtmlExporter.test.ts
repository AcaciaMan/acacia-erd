...
function entityAttr(obj: Record<string, any>): string {
    return JSON.stringify(obj).replace(/"/g, '&quot;');
}

function makeSvg(...entityObjs: Record<string, any>[]): string {
...
function createFsStub(templateContent: string = TEMPLATE_CONTENT) {
    return {
...
function loadHtmlExporter(fsStub: any, vscodeMock: any) {
...
suite('HtmlExporter', () => {
    ...
    suite('extractEntitiesFromSvg()', () => {
        test('extracts entities from SVG with data-entity attributes', () => {
            const svg = makeSvg(
                { id: '1', name: 'User', columns: ['id', 'name'] },
                { id: '2', name: 'Order', columns: ['id'] },
            );
    ...
    suite('exportToHtml()', () => {
        ...
        test('reads template from <extensionPath>/resources/standalone_erd_template.html', async () => {
            ...
            assert.ok(
                readPath.includes('resources') && readPath.includes('standalone_erd_template.html'),
                `Expected template path, got: ${readPath}`
            );
        });
        ...
        test('when user selects save location: writes file, shows success message', async () => {
            ...
            assert.ok(
                (vscodeKit.showInfoStub.firstCall.args[0] as string).includes('test.html')
            );
        });
        ...
        test('on file write error: shows error message, re-throws', async () => {
            ...
            await assert.rejects(
                () => HtmlExporter.exportToHtml('/ext', sampleData),
                (err: Error) => {
            ...
            assert.ok(
                (vscodeKit.showErrorStub.firstCall.args[0] as string).includes('disk full')
            );
        });
    });
});
